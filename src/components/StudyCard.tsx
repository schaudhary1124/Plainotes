...
export function StudyCard({ item }: StudyCardProps) {
  if (item.kind === "flashcard") {
    return <FlashcardView question={item.question} answer={item.answer} reason={item.reason} />;
  }

  return (
    <McqView question={item.question} options={item.options} answer={item.answer} reason={item.reason} />
  );
}
      ...
      >
        ...
        >
          ...
          <div className="flex min-h-0 flex-col items-center gap-2 overflow-y-auto">
            ...
            {reason && <p className="text-secondary max-w-md text-sm @max-md:text-xs">{reason}</p>}
          </div>
  ...
  return (
    <div className="glass-panel shadow-app flex h-72 w-full flex-col gap-5 rounded-2xl p-8 @max-lg:h-56 @max-md:h-48 @max-md:gap-2.5 @max-md:p-4 @max-sm:h-40">
      ...
      <div className="flex flex-1 flex-col gap-2.5 overflow-y-auto">
        ...
        {hasAnswered && reason && <p className="text-secondary text-sm @max-md:text-xs">{reason}</p>}
      </div>
    </div>
  );
}
